...
export const Card = ({ card, index, layout = false, }: { card: Card; index: number; layout?: boolean;}) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      document.body.classList.remove("overflow-hidden"); // Ensure reset on unmount
    };
  ...
  return (
    <>
      <AnimatePresence>
        {open && (
          <div className="fixed inset-0 h-screen z-[1100] overflow-auto">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            <motion.div
              ...
              className="max-w-5xl mx-auto bg-neutral-300 dark:bg-neutral-900 h-fit  z-[1000] my-10 p-4 md:p-10 rounded-3xl font-sans relative overflow-y-auto max-h-[90vh]"
            >
              <button
